...
sep=", "
for arg in "$@"; do
  if [ "$arg" = "SGE_TASK_ID" ]; then
    MATLAB_script_vars="${MATLAB_script_vars}${sep}${SGE_TASK_ID}"
  else
...
mv $HOME/$JOB_NAME.$JOB_ID.$SGE_TASK_ID.out $OUTDIR/$JOB_NAME.$JOB_ID.$SGE_TASK_ID.out

...
MNIs_full = readtable('MNI_coords_5mm.xlsx');
MNIs = MNIs_full(:, contains(MNIs_full.Properties.VariableNames, 'MNI'));
% coil info
dIdtmax = 149.77e6;
%% subject info
for k = 1:height(masterlist)
    % mesh info
    ...
    % run on cluster
    tmp_cluster_loc = fullfile(cluster_loc, subj);
    mkdir(tmp_cluster_loc)
    save(fullfile(tmp_cluster_loc, 'tmp.mat'))
    copyfile(fullfile(out_loc, subj, 'FEM_1', 'Modes_110', 'B_*'), tmp_cluster_loc)
    copyfile(fullfile(out_loc, subj, 'FEM_1', 'Modes_110', 'Q_*'), tmp_cluster_loc)
    system(['ssh ' cluster_user ' qsub PMD-TMS/run_cluster_optimize.sh ' subj]);
    while isempty(dir(fullfile(tmp_cluster_loc, '*.out')))
        pause(60);
    end
    copyfile(fullfile(tmp_cluster_loc, 'E50.xlsx'), fullfile(out_loc, subj))
    copyfile(fullfile(tmp_cluster_loc, '*.out'), fullfile(out_loc, subj))
    system(['rm -r ' tmp_cluster_loc]);
end
function [] = main_offline_stage(pmd_code_path,msh_file,msh_file_read_fcn,msh_file_read_fcn_location,m2m_dir,NModes,FEMORD,...
    ...
    elseif strcmpi(run_mode,'parallel_SGE')
        if ~exist(fullfile('.','SGE_output'),'dir')
            mkdir(fullfile('.','SGE_output'))
        end
        parallel_bash_script_name = fullfile(pmd_code_path,'offline_parallel_SGE_run_script.sh');
        if ispc
            conversion_command = 'unix2dos';
        else
            conversion_command = 'dos2unix';
        ...
        system(cmdStr);
        disp([newline,'Modes are being calculated in the cluster',newline])
    elseif strcmpi(run_mode,'parallel_local')
        offline_parallel_local_run_script(pmd_code_path,NModes,msh_file,msh_file_read_fcn,msh_file_read_fcn_location,m2m_dir,...
    ...
    elseif strcmpi(run_mode,'serial')
        ...
        disp("Please specify whether to run the offline stage in 'parallel', 'parallel_local', or 'serial'")
    end
end
